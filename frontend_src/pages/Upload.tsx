...
export default function Upload() {
  ...
  const handleDownloadReport = () => {
    if (!result || !imageUrl || hazards.length === 0) return
    ...
    doc.setFontSize(12)
    doc.text("Detected hazards:", 20, 30)
    ...
    originalImg.onload = () => {
      ...
      doc.addImage(originalImg, 'JPEG', 20, 30, 160, 120)

      const predictedImg = new Image()
      predictedImg.src = result
      predictedImg.onload = () => {
        doc.addPage()
        doc.setFontSize(16)
        doc.text("Predicted Image", 20, 20)
        doc.addImage(predictedImg, 'JPEG', 20, 30, 160, 120)
